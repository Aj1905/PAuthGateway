...
実装済みの構造は `ARCHITECTURE.md` を正とする(§1 構成図、§1.1 結合境界と帰結、
§2 構成要素、§8 ツール供給元)。Planner の正準名と生成処理は
...
計画時の Planner は差し替え可能である。次の三つは正準名として登録済みだが、
実装されていない。選択すると `PlanGenerationError` が発生し、Gateway は計画生成失敗を、
Enforcer とルールを持たないセッションとして記録する。

| 正準名 | 現在の状態 |
|---|---|
| `interactive-structuring` | 登録済み、未実装 |
| `specialized-codegen` | 登録済み、未実装 |
| `formal-semantic` | 登録済み、未実装 |

各候補の流れ、導入条件、失敗要因は `PLANNING_STRATEGIES.md` に記録する。採用する
場合も、`ARCHITECTURE.md` §1.1 と `GLOSSARY.md` が定める Planner 契約を維持する。
