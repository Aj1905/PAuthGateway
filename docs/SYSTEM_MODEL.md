...
検証済みのrunコードを機械的に読み、ツール呼び出し一つごとの**スライス**
...
Algorithm 1)。決定的。ここまで(Planner〜Rule compiler)が
...
ベンチマークは有限の正解ツール呼び出し列を与え、模擬実行は有限の許可トレース
を生成する。
...
- **過剰 / Excess.** 正解ツール呼び出し列と一致しない許可済みツール呼び出し。
- **不足 / Missing.** 許可トレースと一致しない正解ツール呼び出し。計画の
