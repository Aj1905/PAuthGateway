...
`PromptMessage` / `ToolCallMessage` へ正規化される。これは、捕捉と執行の
通信路上の方向を記述するものでは**ない**。往復の区間モデル
(outbound/return × ingress/egress — プロンプトをどこで観測し、ツール呼び出しを
どこで観測し、執行が働き得る唯一の区間はどこか)は `INGRESS_DESIGN.md` の
「Directional model」で定義される。この二つの語彙は混同しないこと。本書の
"ingress" = アダプタであり、当該文書における復路の egress = 執行の差し込み点
である。両者は同義語ではない。
...
outbound/return × ingress/egress の区間モデルは `INGRESS_DESIGN.md` の
「Directional model」を参照。)
