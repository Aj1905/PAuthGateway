# AVAIL_4 改善記録 — 目標: agentdojo CALLS_MADE → 100%

> **歴史的記録。** 本ファイルは各実験の実施当時に使われていた名称と分母を保
> 存する。現在の分類法では、`AVAIL_2` はpolicy生成に対応し、`AVAIL_3` は実行
> 時クラッシュなしと観測されたplan-policy整合に分割され、`AVAIL_4` は必要
> call充足のみに当たる。`SEC_NO_EXCESS_CALLS` は古い非対称なtrace比較だった。
> これらの歴史的な値を `REF_EXACT_AUTHORIZATION` と読み替えてはならない。
> 過不足なしの認可は、共通の1対1照合器で再計算しなければならない。

**指標。** AVAIL_4_CALLS_MADE = 不足なし: RAN_CLEAN したplanのうち、実行
traceが**必要なground-truth callをすべて**含む(引数も一致)。不足 =
ground-truth callがtraceから欠落している(または誤った引数で呼ばれた)こと。

**基準値(キャッシュ済み one-shot plan)。** agentdojo AVAIL_4 = ran-clean中
**16/51**。
連鎖: EXPRESSIBLE 97/97 ⊇ PLAN_VALID 62/97 ⊇ RAN_CLEAN 51/62 ⊇ CALLS_MADE 16/51.

**本記録の規則。** すべての介入について: AVAIL_4 の増減を測る。固定ラベル付
き攻撃集合の結果(AUX_INJECTIONS_DENIED)は損なわない。変更が AVAIL_4 を下
げるか健全性を壊すなら、差し戻して理由をここに記録する。その上で次の方法を
試す。

---

## 診断(ran-cleanな35planが不足を抱える理由)
ran-cleanだが不足のあるキャッシュ済みplan 35件の内訳:
...
- **結果:** AVAIL_4 16/51 (31%) -> 18/60 (30%)。RAN_CLEAN 51->60、OUTCOME 14->18。
...
- **39件は非制御の不一致** — 無害または責務範囲外の引数が大半:
  ...
  はGTにしかない値)。AVAIL_4 は現在すべての引数で照合するため、PAuth の責
  務が制御オペランドにあるにもかかわらず、これらが不足として数えられる。
つまり AVAIL_4 の上限は、主として責務範囲外の引数への厳格さによって決まって
おり、PAuth が誤った動作をしているからではない。

### T3 — AVAIL_4 を制御オペランドのみで照合(測定の変更であり Planner の変更ではない)
- **仮説:** AVAIL_4 は、必要callの制御オペランド(recipient/amount)が一致
...
- **方法:** `_deficiency_control` を追加(`control_operands` による制御オペ
  ランド添字でGT callを照合)し、gates.py + funnel.py の AVAIL_4 に組み込む。
  過剰(SEC_NO_EXCESS)は厳格な全引数照合を維持。
- **結果:** AVAIL_4 **16/51 (31%) -> 26/51 (51%)**(banking 2->5、slack 1->3、
  travel 0->3、workspace 13->15)。FN=0 は維持(62/62)。269件のテストが通過。
- **判定:** 採用 — 原則に適う(AVAIL_4 を PAuth の制御責務に整合させる)も
...
- **結果:** AVAIL_4 31/60 (52%) 対 キャッシュ+制御照合 26/51 (51%)。FN=0(64/64)。
...
- **方法:** 候補N=3、文法的に妥当 + clean + 副作用call最大のものを選択。
  AVAIL_4(制御照合)を測定。banking、作業領域を再利用。
- **結果:** banking AVAIL_4 **5/14 (36%) -> 10/16 (63%)**。大幅な上昇 — 中身
  のない候補ではなく行動する候補を選べている。
- **判定:** banking では勝者(正当な手段。ground truth 不使用)。全体へ拡大する。

### T6 — best-of-N を agentdojo 全体へ拡大
- **結果:** AVAIL_4 **36/69 (52%)**(PLAN_VALID 62->77、RAN_CLEAN 51->69、
  FN=0 77/77)。banking の63%は一般化しなかった — 全体の比率はキャッシュ+制
  御照合(51%)とほぼ横ばい。best-of-N は PLAN_VALID/RAN_CLEAN(動くplanの
...
- **判定:** best-of-N は可用性(PLAN_VALID/RAN_CLEAN)を助けるが、AVAIL_4
  の比率は上げない。Planner の選択肢として採用する。AVAIL_4 を引き上げると
  主張してはならない。
...
- **結果:** AVAIL_4 **40/66 (61%)** 対 bestofのみ 36/69 (52%)、+9pt。FN=0(75/75)、
...
- **結果:** AVAIL_4 **23/68 (34%)** — bestof+structuring(61%)より悪化。COST
...
- **結果:** AVAIL_4 **40/65 (62%)** 対 best-of-3(T7)40/66 (61%)。横ばい
...
- **結果:** PLAN_VALID 62->**92/97**、RAN_CLEAN 51->**88/92**、OUTCOME 14->22、
  FN=0(92/92)。AVAIL_4 **47/88(比率53%、97全体の48%)** 対 gpt-4.1 の比率
...
## 結論 — AVAIL_4 = 100% は到達不能(複数モデルで証明)
**最良の比率: AVAIL_4 31% -> 61%**(制御オペランド照合 + best-of-N +
structure_text、gpt-4.1、T7)。より強いモデル(gpt-5.1、T10)は可用性を大幅
に引き上げた(PLAN_VALID 62->92、OUTCOME 14->22)が、AVAIL_4 の比率(53%)
...
- **仮説:** (より強い)gpt-5.1 なら、judge が reject sentinel に落ちる代わ
  ...
  しれない。
- **結果:** AVAIL_4 **23/95 (24%)**、COST **0.9 calls/task** — judge は
  gpt-5.1 でもplanを空洞化させる(0.9 call = ほぼ `pass`)。PLAN_VALID 95/97
  は、より多くのplanがjudgeにかけられ却下される → さらに空洞化することを意
  味する。FN=0(95/95)。
- **判定:** 失敗(悪化)。judge は両モデルで AVAIL_4 に対して系統的に逆効果
  — reject sentinel への退避が支配する。モデルの問題でないことを確認。差し
  戻し済み(--judge オフ)。

### T12 — 選択方針(最多行動 対 最小完備)、Planner は固定
- **仮説:** best-of-N の「副作用が最も多いclean候補を選ぶ」方針こそが
  AVAIL_4 と過剰の両方を膨らませているのかもしれない。「副作用call数が0より
  大きい中で最少」(最小完備)で選び直せば、AVAIL_4 を少し譲って最小権限を
...
- **結果(gpt-5.1 struct、/97):** 最大と最小はほぼ一致。
  AVAIL_4 47->46、SEC_NO_EXCESS **21->21(変化なし)**、OUTCOME 22->20、COST 3.17->2.99。
  診断: clean候補を持つ88タスクのうち、副作用call数が異なるのは15件のみで、
  行動しつつ過剰の少ない代替候補を選べるのは**8件**のみ。
- **判定:** 操作手段としては失敗。曲線は実質的に一点 — N=3 では選択方針は
  AVAIL_4 も最小権限も動かさない。**以前の主張の訂正:** 過剰は選択の産物で
  はなく、Planner に内在する(N個の候補すべてが同じ過剰callを出力する)。選
  び直しでは AVAIL_4 を上げることも過剰を下げることもできない。

### T13 — 別フレームワークでの AVAIL_4(tau retail、gpt-5.1)= アーキテクチャ上の上限
- **観察(介入ではない):** tau retail AVAIL_4 = **0/79**。tau のタスクはす
...
- **数字いじり防止の注記:** 照合をさらに緩めるか、中身のないplanを分母から
  除外すれば、AVAIL_4 は自明に100%へ強制できる。しかしそれは指標の数字いじ
