...
本書は PAuthGateway の性能評価についてまとめる。
本書のすべての数値は、各節に記載したコマンドから再現できる。

本ソフトウェアの目的に即した評価指標を定義し、自作データセットや評価フレームワークを利用して性能を定量化する。

---
## 評価指標の定義

本ソフトウェアは、ユーザーのエージェント利用体験を大きく変えることなくタスクごとの最小権限付与を自動化し安全性を向上させることである。

よって、測るべき指標は以下の4種に区分される。
