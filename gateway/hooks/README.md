...
| `PAUTH_CONFIRMATION_POLICY` | `reject` / `approve` / `human` | `human` | 保留の判断方針。`reject` = 全拒否(人間が確認しない場合の安全側の既定)、`approve` = 全実行(比較条件 — 本番の既定にしない)、`human` = 確認面で人間が判断。自動方針では UX の版は結果に影響しない。 |
