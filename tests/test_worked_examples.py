...
output into the deterministic pipeline (the Slicer, the RuleCompiler, runtime enforcement) and checks that
...
def run_and_check(suite, task: TaskSpec) -> None:
    ...
    check(result.usable, f"[{task.id}] the Planner->the RuleCompiler produced a checkable task", result.planner_detail)
