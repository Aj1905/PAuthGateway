...
1. **the Planner** -- generate the imperative ``run`` function (LLM, or a cached /
   reference copy).
2. **the Slicer / the RuleCompiler** -- derive slices and compile rules (deterministic).
...
class TaskResult:
    ...
    def usable(self) -> bool:
        """True if the Planner+the Slicer+the RuleCompiler produced a checkable task."""
...
def run_task(
    ...
    """Run the full the Planner -> the RuleCompiler -> B pipeline for a single task.
    ...
    # ---- the Slicer / the RuleCompiler: slices and rules ------------------------------------
    try:
